...
# 总结
[1]  使用union all 替代 or （or 全表查询）   
[2]  尽量避免全表扫描 考虑在where 及 order by 涉及的列上建立索引
[3]  避免在 where 子句中使用 != 或 <> 操作符，（ != 或 <> 操作符  全表查询）
[4]  避免在 where 子句中对字段进行 null 值判断（否则将导致引擎放弃使用索引而进行全表扫描）（最好不要给数据库留NULL，尽可能的使用 NOT NULL填充数据库）
[5]  不要在 where 子句中的“=”左边进行函数、算术运算或其他表达式运算，否则系统将可能无法正确使用索引
[6]  使用exists 替代 in  例如：select num from a where exists(select 1 from b where num=a.num)
      exists通俗的可以理解为：将外查询表的每一行，代入内查询作为检验，如果内查询返回的结果取非空值，则EXISTS子句返回TRUE，这一行行可作为外查询的结果行，否则不能作为结果。
      ...
      【2】not exists：如果主查询表中记录少，子查询表中记录多，并有索引。
...
[7]  一个表的索引数最好不要超过6个，若太多则应考虑一些不常使用到的列上建的索引是否有 必要。
